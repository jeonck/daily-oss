...
매일 아침(KST 07:00) RSS · Reddit · Hacker News · GitHub에서 새로 알려지고 있는
...
| 판정 | 의미 | 예 |
|---|---|---|
| 🔥 즉시조치 | 지금 바로 써볼 가치가 큰 신규/급부상 오픈소스 | 스타 급증 중인 신규 AI 에이전트 프레임워크, 대체 가능한 핵심 인프라 도구 |
| 📌 백로그 | 유용하지만 급하지 않음, 나중에 검토 | 개선된 CLI 도구, 특정 스택용 라이브러리 |
| 📚 학습 | 당장 도입 대상은 아니나 트렌드/기술 파악용 | 새로운 아키텍처 패턴을 보여주는 프로젝트, 실험적 도구 |
| 무관 | 그 외 전부 → **포스트 생성 안 함** | 홍보성 글, 관심 영역과 무관한 프로젝트 |

각 포스트는 `근거`(어떤 영역-개발자/DevOps/AI/기획/프론트엔드/백엔드/시스템 운영/인프라-에 해당하는지)와
`액션`(이번 주 안에 해볼 구체적 작업 1개, 예: "레포 클론해서 README 대로 실행해보기")을 담는다.
...
hugo server        # → http://localhost:1313/daily-oss/
```
